...
<body>
    ...
    // これは id を元に、すべてのカラムを引き出す関数ですが、前ページですべてのカラムは既に受け取っており、
    // DBアクセスの回数を増やす手段となります。ので、前ページから post + session で受け取る方法に変更致しました。
    session_start();
    // ----add----
    // UPDATE処理はDBを書き換えてしまう重要な操作なので、不正ルートアクセスを弾くように変更致しました。
    ...
    // *************************************** 登録ボタンを押して再読み込み時 ************************************************
        ...
        // ----add----
        // 未入力の値の項目のカラーを赤色へ変更する処理を追加致しました。
