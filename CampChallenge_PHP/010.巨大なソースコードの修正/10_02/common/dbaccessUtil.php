<?php
// ----retouch----
// db_connect() > return_top() で必要になるため require を追加
...
function db_connect() {
    try {
        ...
        // ----retouch----
        // die してしまうと、ここで全ての処理が終了されてしまうため、トップへ戻るが表示されなくなってしまう。
...
function serch_profiles($name=null,$year=null,$type=null){
    ...
    // if (isset($type) and $flag == false) {
    //     $search_sql .= " WHERE";
    //     foreach ($type as $key => $value) {
    //         $search_sql .= " type".$key." = :type".$key;
    //     }
    //
    // } elseif (isset($type)) {
    //     $search_sql .= " AND";
    ...
    //クエリとして用意
    // ----retouch----
    // $seatch_query -> $search_query へ修正。スペルミス？
    $search_query = $search_db->prepare($search_sql);
